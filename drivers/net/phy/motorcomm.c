...
static int yt8521_config_init(struct phy_device *phydev)
{
	...
	if (val < 0)
		return val;
	printk (KERN_INFO "zhirong..8521 reg(0xa012) = 0x%x\n", val);
	val |= (1 << 5);
	...
	printk (KERN_INFO "yzhang..8521 init call out(reg(0xa012) = 0x%x)...\n", val);
	return ret;
}
...
static int yt8618_config_init(struct phy_device *phydev)
{
	...
	printk (KERN_INFO "yzhang..8618 init call out...\n");
	return ret;
}
...
#if 0 /*it should be used for 8614 fiber*/
	if((32 == link_mode_8521) && (SPEED_100 == phydev->speed))
	{
		return 1/*link_mode_8521*/;
	}
...
static int yt8614_read_status(struct phy_device *phydev)
{
	...
	{
		link = 0;
		printk (KERN_INFO "yzhang..8614 fiber link down detect,latch=%04x,curr=%04x\n", yt8614_fiber_latch_val,yt8614_fiber_curr_val);
	}
...
#if 0 /*(YT8521_PHY_MODE_CURR != YT8521_PHY_MODE_UTP)*/
	/* reading Fiber */
	ret = ytphy_write_ext(phydev, 0xa000, 2);
	if (ret < 0)
		return ret;
